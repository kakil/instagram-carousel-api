...
class TestImageServiceParametrized:
    ...
    @pytest.mark.parametrize(
        "service_type",
        [ImageServiceType.STANDARD.value, ImageServiceType.ENHANCED.value],
    )
    def test_service_creation(self, service_type, monkeypatch):
        ...
        mock_font = MagicMock()
        monkeypatch.setattr(
            "app.services.image_service.base_image_service.ImageFont.truetype",
    ...
    @pytest.mark.parametrize(
        "test_image_service, expected_behavior",
        [
    ...
    def test_error_handling_behavior(self, test_image_service, expected_behavior):
        ...
        if expected_behavior:
            ...
            result = test_image_service.create_slide_image(
                "Error Test", problematic_text, 1, 1, False, None
            )
    ...
    @pytest.mark.parametrize(
        "title,text,num,total,expected_dimensions",
        [
    ...
    def test_slide_dimensions(
        self, enhanced_image_service, title, text, num, total, expected_dimensions
    ):
        """Test that slides always have the expected dimensions."""
        # Create slide
        img = enhanced_image_service.create_slide_image(title, text, num, total, False, None)
    ...
    @pytest.mark.parametrize(
        "special_text,expected_replacements",
        [
            ("Text with arrow →", "->"),
    ...
    def test_carousel_image_count(self, enhanced_image_service, slides_data, expected_count):
        ...
        result = enhanced_image_service.create_carousel_images(
            "Test Carousel", slides_data, "test123"
        )
