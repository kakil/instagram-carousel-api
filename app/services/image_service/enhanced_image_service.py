"""
Enhanced image service module for the Instagram Carousel Generator.

This module provides an advanced implementation of the image service with
improved error handling, better text rendering capabilities, and more
user-friendly error displays for carousel images.
"""
import logging
import os
import traceback
...
class EnhancedImageService(BaseImageService):
    ...
    It includes better text rendering and more user-friendly error displays.
    """
    ...
    def create_gradient_text(
        ...
        try:
            ...
            gradient_text = self._create_gradient_text_image(
                text, font, text_width, text_height, colors
            )

            # Calculate centered position
            centered_position = self._calculate_centered_position(position, text_width, text_height)
    ...
    def _create_empty_transparent_image(
    ...
    def _get_text_dimensions(self, draw: ImageDraw.Draw, text: str, font) -> Tuple[int, int]:
    ...
    def _calculate_centered_position(
        self, position: Tuple[int, int], text_width: int, text_height: int
    ) -> Tuple[int, int]:
    ...
    def _create_fallback_text(
        self, text: str, font, width: int, position: Tuple[int, int]
    ) -> Tuple[Image.Image, Tuple[int, int]]:
        """Create fallback text when gradient rendering fails."""
        logger.error("Error in create_gradient_text")  # Removed f-string with no placeholders
    ...
    def create_error_slide(
        ...
        self._add_simple_error_message(draw, width, height, simple_error, fonts["body"])

        # Add detailed error message
        self._add_detailed_error_info(draw, width, height, error_message, fonts["small"])

        # Add help instruction
        self._add_error_instruction(draw, width, height, fonts["body"])

        # Add slide counter
        self._add_slide_counter(draw, slide_number, total_slides, fonts["small"], width, height)
    ...
    def _load_error_slide_fonts(self) -> Dict[str, ImageFont.FreeTypeFont]:
        ...
        return {
            "title": self.safe_load_font(self.settings.get("title_font", "Arial Bold.ttf"), 48, 32),
            "body": self.safe_load_font(self.settings.get("text_font", "Arial.ttf"), 32, 24),
            "small": self.safe_load_font(self.settings.get("nav_font", "Arial.ttf"), 24, 18),
        }

    def _add_error_title(
        self,
        draw: ImageDraw.Draw,
        width: int,
        height: int,
        ...
        draw.text(text_position, title, fill=(255, 100, 100), anchor="mm", font=font)
    ...
    def _add_simple_error_message(
        ...
        draw.text(text_position, simple_error, fill=(240, 240, 240), anchor="mm", font=font)

    def _add_detailed_error_info(
        self,
        draw: ImageDraw.Draw,
        width: int,
        height: int,
    ...
    def create_slide_image(
        ...
        text_font = self.safe_load_font(self.settings.get("text_font", self.default_font), 48, 36)
        ...
        if sanitized_title:
            self._add_title_to_slide(image, draw, sanitized_title, title_font, width)

        if sanitized_text:
            self._add_text_to_slide(image, draw, sanitized_text, text_font, width, height)

        self._add_navigation_to_slide(
            draw, slide_number, total_slides, navigation_font, width, height
        )

        if include_logo and logo_path:
            self._add_logo_to_slide(image, logo_path, width, height, navigation_font)
    ...
    def _add_navigation_to_slide(
        self, draw, slide_number, total_slides, navigation_font, width, height
    ):
        ...
        self._add_slide_counter(draw, slide_number, total_slides, navigation_font, width, height)

    def _add_slide_counter(self, draw, slide_number, total_slides, navigation_font, width, height):
    ...
    def _add_logo_to_slide(self, image, logo_path, width, height, navigation_font):
