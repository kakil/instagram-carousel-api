...
def enhanced_sanitize_text(text):
    """
...
def safe_load_font(font_path, size, fallback_size=None):
    """
...
def create_enhanced_error_slide(slide_number, total_slides, error_message,
...
def create_slide_image(
    ...
    Args:
        ...
        slide_number: Current slide number
        total_slides: Total number of slides
        settings: Dictionary containing settings (width, height, bg_color, etc.)
    ...
    bg_color = settings.get('bg_color', (18, 18, 18))
    ...
    title_font = safe_load_font(settings.get('title_font', 'Arial Bold.ttf'),
                                60, 48)
    text_font = safe_load_font(settings.get('text_font', 'Arial.ttf'), 48, 36)
    navigation_font = safe_load_font(settings.get('nav_font', 'Arial.ttf'), 36,
                                     24)

    # Sanitize text content
    sanitized_title = enhanced_sanitize_text(title) if title else None
    sanitized_text = enhanced_sanitize_text(text)
    ...
    if sanitized_title:
        try:
            ...
            try:
                ...
                draw.text((width // 2 - title_width // 2, 150), sanitized_title,
    ...
    try:
        ...
        draw.text((width / 2, height - 60),
                  counter_text, fill="white", font=navigation_font, anchor="mm")
...
def create_carousel_images(
    ...
    Args:
        ...
        settings: Dictionary of settings for image generation
        include_logo: Whether to include a logo
        logo_path: Path to the logo file
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for index, slide in enumerate(slides_data):
            try:
                ...
                # Create a better error slide
                error_img = create_enhanced_error_slide(
                    index + 1,
                    len(slides_data),
                    str(e),
                    settings.get('width', 1080),
        ...
        logger.info(
            f"Carousel generation completed with {len(image_files)} slides")
        return image_files